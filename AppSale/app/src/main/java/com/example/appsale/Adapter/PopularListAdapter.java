...
public class PopularListAdapter extends RecyclerView.Adapter<PopularListAdapter.Viewholder> {
    ...
    public void onBindViewHolder(@NonNull PopularListAdapter.Viewholder holder, int position) {
        holder.tvNameProduct.setText(items.get(position).getTitle());
        holder.tvPriceProduct.setText("$"+items.get(position).getPrice());
        holder.tvStar.setText(""+items.get(position).getScore());

        int drawableResourceId = holder.itemView.getResources()
                .getIdentifier(items
                        .get(position)
                        .getPicUrl(),"drawable",holder
                        .itemView.getContext()
                        .getPackageName());
    ...
    public class Viewholder extends RecyclerView.ViewHolder{
        TextView tvNameProduct, tvPriceProduct, tvStar;
